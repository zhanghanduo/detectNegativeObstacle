...
bool CRectangleDetector::checkShape(cv::Point pt0, cv::Point pt1, cv::Point pt2, cv::Point pt3)
{
    double dx1 = fabs(pt1.x - pt0.x);
    double dy1 = fabs(pt1.y - pt0.y);
    double dx2 = fabs(pt3.x - pt2.x);
    double dy2 = fabs(pt3.y - pt2.y);
    double dy = fabs(pt3.y - pt0.y);
//    return ((dy2< 20)&&(dy1 < 20)&&(dx1/dy < 15));
//    return ((dy2< 32)&&(dy1 < 33)&&(dx1/dx2 < 2.6)&&(dx1/dx2 > 0.8));
    return ((dy2< 43)&&(dy1 < 43));
}
...
//	{
		FilterByMaxSize(colorSquares[0], filteredBySizeSquares[0], maxsize);
//	}
}
		...
		vector<vector<cv::Point> > &squares)
{
	...
	colorRange.push_back(cv::Scalar(185,185,185));
...
//	{
		...
		colorSquares[0].clear();
		copy(filteredBySizeSquares[0].begin(), filteredBySizeSquares[0].end(), back_inserter(colorSquares[0]));
//	}
...
//    {
        ...
        for( int l = 0; l < m_numThreshIters; l++ )
        {
            ...
            for( size_t i = 0; i < contours.size(); i++ )
            {
            	...
            	cv::approxPolyDP(cv::Mat(contours[i]), approx, cv::arcLength(cv::Mat(contours[i]), true)*0.007, true);
//                cout << "size: " << approx.size() << endl;
                ...
                // contour orientation
                if( approx.size() == 4 &&
                    fabs(cv::contourArea(cv::Mat(approx))) > 1300 &&
//                        cv::isContourConvex(cv::Mat(approx)))
                    cv::isContourConvex(cv::Mat(approx)) && (checkShape(approx[0],approx[1],approx[2],approx[3])) )
                {
//                    double maxCosine = 0,minCosine = 0;

//                    for( int j = 2; j < 5; j++ )
//                    {
//                        // find the maximum cosine of the angle between joint edges
//                        double cosine = angle(approx[j%4], approx[j-2], approx[j-1]);
//                        maxCosine = MAX(maxCosine, cosine);
//						minCosine = MIN(minCosine, cosine);
//                    }
                    double cosine1 = angle(approx[3], approx[1], approx[0]);
                    double cosine2 = angle(approx[0], approx[2], approx[1]);
//                    double cosine3 = angle(approx[3], approx[1], approx[2]);
                    double cosine4 = angle(approx[2], approx[0], approx[3]);
//                    cout << "1: " << cosine1 << "2: " << cosine2 << "3: " << cosine3 << "4: " << cosine4 << endl;
//                    if( ( (cosine1 > 0.08 ) && (cosine4 < -0.08) ) || ( (cosine1 < -0.08 ) && (cosine4 > 0.08) &&(cosine2 > 0.1) ) ){
                    if( ( (cosine1 > 0.08 ) && (cosine4 < -0.08) ) || ( (cosine1 < -0.08 ) && (cosine4 > 0.08)  ) ){
                        colorSquares[0].push_back(approx);
//                        cout << "1: " << approx[0] << "2: " << approx[1] << "3: " << approx[2] << "4: " << approx[3] << endl;
                    }

                    // if cosines of all angles are small
                    // (all angles are ~90 degree) then write quandrange
                    // vertices to resultant sequence
//                    if(( minCosine < - 0.1 ) && ( maxCosine > 0.1))
...
//	const cv::Scalar CV_GREEN(0, 255, 0);
    ...
    cv::Point offset(0, 510);
    for( size_t i = 0; i < squares[0].size(); i++ )
    {
        const cv::Point* p = &squares[0][i][0];
//        p->y = p->y + 490;
        int n = (int)squares[0][i].size();
//        cv::polylines(image, &p, &n, 1, true, CV_RED, 3, CV_AA);
        cv::fillPoly(image, &p, &n, 1, CV_RED, CV_AA, 0, offset);
    }
    return image;
}
